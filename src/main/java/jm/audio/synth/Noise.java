...
public class Noise extends AudioObject {
    ...
    switch (this.noiseType) {
      ...
      case GENDYN_NOISE:
        ...
        for (; ret < buffer.length; ) {
          ...
          if (gendynIntArrayCounter >= (int) gendynIntArrayLength) {
            makeGendynArray();
          }
        }
        break;
  ...
  private double randWalk(final double prevVal, final double stepSize, final int mirror,
    ...
    if (gendynGaussian) {
      ...
      rwNewVal = prevVal + (RandomGenerator.nextDouble() * stepSize * 2.0 - stepSize);
    }
    // keep inside range
    if (timeWalk) {
      if (stepSize == 0) {
        ...
        while (rwNewVal > mirror || rwNewVal < 0.0) {
          if (rwNewVal > mirror) {
            rwNewVal = mirror - (rwNewVal - mirror);
          }
          if (rwNewVal < 0.0) {
            rwNewVal = (rwNewVal / 2.0 * -1.0);
          }
        }
      }
      ...
      while (rwNewVal > mirror || rwNewVal < negMirror) {
        ...
        if (rwNewVal < negMirror) {
          rwNewVal = negMirror + (negMirror - rwNewVal);
        }
      }
      // avoid zero ????
      if (rwNewVal < 0.0) {
  ...
  private void updateGranularity() {
    if (tempGendynGranularity > 0) {
      this.gendynAmpGranularity = tempGendynGranularity;
    }
  }
