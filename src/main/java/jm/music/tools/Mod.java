...
public class Mod implements JMC {
  ...
  public static void diatonicInvert(Phrase phrase, int[] scale, int root) {
    ...
    while (i < phrase.size() && firstNotePitch == Note.REST) {
      ...
      if (root == 0) {
        firstNoteOctave = firstNotePitch / 12;
      } else {
        firstNoteOctave = (firstNotePitch - 12 + root) / 12;
      }
    ...
    for (; i < phrase.size(); i++) {
      ...
      if (iSteps < 0) {
        ...
        if (nextStep < 0) {
          nextStep = scale.length + nextStep;
        }
        ...
        if (octCalc >= 0) {
          ...
          nextOctave = firstNoteOctave + (octCalc + 1) / scale.length - 1;
        }
      }
      // change the pitch to invert each around the first note
